...
class FusionNetNoWiFiEncoder(nn.Module):
    """多模态融合模型Encoder，融合WiFi和视频特征进行人数估计。
    使用Transformer Encoder和CLS Token的模式。
    """
    ...
    def __init__(
        ...
        self.fusion_video_scheme = fusion_config.get("fusion_video_scheme", 5)
        self.video_list = fusion_config.get("video_indices", [3, 5, 4, 2, 1])
        self.output_size = fusion_config.get("output_size", 1)

        # 可学习CLS向量
        self.cls_token = nn.Parameter(torch.empty(1, 1, 700))
        ...
        # 初始化融合层
        self.fcvideo = nn.Linear(512, 700)
        self.tse = nn.TransformerEncoderLayer(d_model=700, nhead=10, dim_feedforward=2048, batch_first=True)
        self.avgpool = nn.AdaptiveAvgPool1d(1)
        
        # FC层输入维度为CLS Token的维度
    ...
    def forward(self, x: Tuple[torch.Tensor, torch.Tensor]) -> torch.Tensor:
        ...
        for i in range(self.fusion_video_scheme):
            ...
            video_feature = resnet.tse(video_feature.flatten(2).transpose(1, 2))
            video_feature = self.fcvideo(video_feature)
        ...
        # 3. 添加CLS Token并输入Transformer Encoder
        cls_tokens = self.cls_token.expand(batch_size, -1, -1)
        input_seq = torch.cat((cls_tokens, all_video_features), dim=1)
        encoder_output = self.tse(input_seq)
        
        # 4. 取出CLS Token的输出用于预测
        cls_output = encoder_output[:, 0]

        # 5. 最终预测
        fusion_fs = F.relu(self.fc1(cls_output))
        fusion_fs = F.relu(self.fc2(fusion_fs))
        fusion_fs = self.fc3(fusion_fs)
