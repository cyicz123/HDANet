...
class FusionNetDecoder(nn.Module):
    """多模态融合模型，融合WiFi和视频特征进行人数估计，采用encoder-decoder架构"""
    ...
    def __init__(
        ...
        # Recalculate input features for fc1
        # The output of decoder has shape (B, 16, 700)
    ...
    def forward(self, x: Tuple[torch.Tensor, torch.Tensor]) -> torch.Tensor:
        ...
        # 3. 将wifi特征和所有视频特征堆叠成 memory
        # memory = torch.cat([wifi_f, all_video_features], 1)
        
        # 4. 使用 Transformer 解码器处理特征, wifi_f is query
        decoded_features = self.tsd(wifi_f, all_video_features)
